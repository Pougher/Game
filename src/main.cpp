...
int main() {
    ...
    state->shaders["default"].set_mat4("projection",
    ...
    while (!glfwWindowShouldClose(state->window.window)) {
        ...
        state->shaders["default"].set_mat4("view", cam->view_matrix());
        state->world.render();
